...
from detectModels.models.common import Conv, DWConv
from detectModels.utils.google_utils import attempt_download
...
def attempt_load(weights, map_location=None):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        # 讀取 .bin 檔
        with open(weights, "rb") as fff:
            weight_bytes = fff.read()

        # 轉為 BytesIO 給 torch.load 使用
        buffer = io.BytesIO(weight_bytes)
        # weights = torch.load(buffer, map_location="cpu")
        ckpt = torch.load(buffer, map_location=map_location, weights_only = False)  # load
